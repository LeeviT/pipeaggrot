...
int main() {
    ...
    for (int i = 0; i <= _params.getny(); i++) {
        ...
        _y_tmp.push_back(N_VNew_Serial(_system_size));
        ...
        CVodeInit(_cvode_mem.at(i), wfunc, _time, _y0.at(i));
        ...
        SUNLinearSolver _lin_sol = SUNLinSol_SPGMR(_y_tmp.at(i), 0, 0);
        CVodeSetLinearSolver(_cvode_mem.at(i), _lin_sol, _jacobian);
