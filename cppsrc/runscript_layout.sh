...
executable=/u/83/tuikkal1/unix/csm/outd_ao_model_pack/run_control_vol_acorrected.exe
#INPUT: executable phi_grid theta_grid diffusion_coeff shear_rate  aspect_ratio dt max_time s1 s2 classes what_todo visco_0 Np_max
#what_todo::
#-1 = selfcheck
# 0=new run
...
for diffusion_coeff in  10.0; do #loop1
...
mkdir sr_${sr}
cd sr_${sr}
$executable $phi_grid $theta_grid $diffusion_coeff $sr $aspect_ratio $dt $max_time $s1 $s2 $classes $what_todo   > output.dat
...
#for folder in sr_*; do echo $folder | sed s/sr_// | tr "\n" " "; tail -1 ${folder}/viscoraw.dat; done > last_viscoraws.dat
