...
void Combo_class::initialize(double shear_rate_, input params_) {
    ...
    if (M != 1 && N != 1) { //If this is false forget about the orientation part
        for (indexb i = 0; i < classes; i++) {
	        double _diffusion_coeff = diffusion_coeff_max * (i + 1.0) / (classes * 1.0);
...
// After this function the distribution stored in Combo_class is the one given as input *y_now
void Combo_class::get_dt(double * y_dot_, double * y_now_) {
    ...
    // vCopy the calculated dist_dt (multiarray) to y_dot ...
...
double Combo_class::get_visc_raw(int s1_, int s2_, int timestep_, int r_i_) {
    for (int i = 0; i < classes; i++) {
        multi_array_ref<double, 2>::array_view<2>::type _tempView_wdist = distribution[indices[i][range(0, N)][range()]];
        if (timestep_ != 0) orientation_class[i].dist_avgs(_tempView_wdist, i, timestep_);
    }
...
double Combo_class::get_tau_raw(int s1_, int s2_) {
    ...
    d2vec _tau_raw(boost::extents[3][3]), _tau_raw_i;
    ...
    for (int i = 0; i < classes; i++) {
        ...
        _tau_raw_i = orientation_class[i].get_tau_raw(distribution[indices[i][range(0,N)][range()]], shear_rate);
...
void Combo_class::update_shear_rate(double shear_rate_) {
    ...
    // Update shear rate for aggregation class if aggregation is used
    if (classes > 1) aggregation_class.update_v_aggregation(shear_rate_);
    // Update shear rate for orientation class(es) if more than one populations exist
    if (M != 1 && N != 1) {
        for (indexb i = 0; i < classes; i++) {
