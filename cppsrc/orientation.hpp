...
private:
    ...
    double k_aspect, diffusion_coeff, shear_rate, h_phi, h_theta, tot_area;
...
public:
    ...
    // Initialize the matrices, has to be called each time the shear rate changes
    void initialize_abcde(double shear_rate_);

    // Update variable shear_rate
    void update_shear_rate(double shear_rate_) { shear_rate = shear_rate_; }
