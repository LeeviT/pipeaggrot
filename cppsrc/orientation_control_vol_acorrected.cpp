...
pair<double, double> Orientation_ao::dot_p_angles(double phi_, double theta_){
    //x y z 
    triple r_;
    ...
    kappa[s2][s1]=kappa_t[s1][s2]=shear_rate; 
    ...
    dot_p_s.second  = lambda_m*kt_rp + 
        lambda_p*k_rp;
    dot_p_s.first  = lambda_m*kt_rt + 
        lambda_p*k_rt;
    return dot_p_s;
}
//initialize these -- 
...
void Orientation_ao::initialize_abcde()
{
    ...
    for(int i=0; i!=N; ++i){
        ...
        for(int j=0; j!=M; ++j){
            ...
            W_ipj[i][j] =   
                ( -sin(theta_)/ 2*dot_p_angles(phi_p,theta_).second / sin(theta_) //orientation part
                + diffusion_coeff / (sin(theta_)*h_phi) )  //diffusion part
                //~ /(sin(theta_)*h_phi);
                * (h_theta/area[i][j]);
            W_imj[i][j] = 
                ( +sin(theta_)/ 2*dot_p_angles(phi_m,theta_).second / sin(theta_) //orientation part
                + diffusion_coeff / (sin(theta_)*h_phi) )  //diffusion part
                //~ /(sin(theta_)*h_phi);
                *(h_theta/area[i][j]);
            W_ijp[i][j] =  
                ( -sin(theta_p)/ 2*dot_p_angles(phi_,theta_p).first //orientation part
                + diffusion_coeff * sin(theta_p) / h_theta ) //diffusion part
                //~ /(sin(theta_)*h_theta);
                *(h_phi/area[i][j]);
            W_ijm[i][j] = 
                ( +sin(theta_m)/2*dot_p_angles(phi_,theta_m).first
...
void Orientation_ao::selfcheck(){
    ...
    for(int i=0; i!=N; i++){
        for(int j=0; j!=M; j++){
            cout << (phi[i]/(2*PI))*360 << " " << (theta[j]/(2*PI))*360 << " "<< (dot_p_angles(phi[i], theta[j])).second << endl;
        }
    }
    cout << "\n\n";
    for(int i=0; i!=N; i++){
        for(int j=0; j!=M; j++){
            cout << (phi[i]/(2*PI))*360 << " " << (theta[j]/(2*PI))*360 << " " << (dot_p_angles(phi[i], theta[j])).first << endl;
        }
    }
    cout << "\n\n";
    for(int i=0; i!=N; ++i){
    ...
    double dt_unity=0;
    int N_1=N-1, M_1=M-1;
    int i, j, im1, ip1, jm1,jp1, i_special;
    for(i=0;i!=N;++i){
        im1=i-1; ip1=i+1; if(!i) im1=N-1; else if(i==N_1) ip1=0;
        if(i+N/2 < N) i_special=i+N/2;
        else i_special=i-N/2;
        for(j=1; j!=M_1; j++){
            jm1=j-1; jp1=j+1;
    ...
    for(i=0;i!=N;++i){
        for(j=0; j!=M; j++){
            triple temp(phi[i],theta[j]);
            pair<double, double> temp_dt_angles=dot_p_angles(phi[i],theta[j]);
